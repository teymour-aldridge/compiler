...
fn test_invalid_operator() {
    insta::assert_snapshot!(ui_test("(a b c)".to_string()))
}

#[test]
/// I'm also reasonably happy with this error message
fn test_invalid_brackets_1() {
    insta::assert_snapshot!(ui_test("( ( ) a".to_string()))
}

#[test]
fn test_invalid_brackets2() {
    insta::assert_snapshot!(ui_test("f(x + a * z (( zz".to_string()))
}

#[test]
fn test_invalid_brackets3() {
    insta::assert_snapshot!(ui_test("(((((((()".to_string()));
    insta::assert_snapshot!(ui_test("(((((((())".to_string()));
    insta::assert_snapshot!(ui_test("(((((((()))".to_string()));
}
...
fn test_non_alphabetic_ident() {
    ...
    insta::assert_snapshot!(ui_test("0x = 15".to_string()));
}

#[test]
fn test_missing_endfor() {
    // todo: make this highlight the *whole* token (see the debug snapshot for specifics)
    insta::assert_snapshot!(ui_test(
        include_str!("ui-examples/missing-next").to_string()
    ))
}
